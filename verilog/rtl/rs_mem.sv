  ...
  always_comb begin
    ...
    for (int i = 0; i < RS_MEM_DEPTH; i++) begin
      ...
      if (v.woken[i].valid) begin
        ...
        end else if (!v.oldest_found[1] || (v.cand_age < v.best_age[1])) begin
    ...
    for (int p = 0; p < MEM_ISSUE_WIDTH; p++) begin
      if (v.oldest_found[p]) begin
        ...
        v.oldest_ready[p] = v.woken[v.oldest_idx[p]].src1_ready && v.woken[v.oldest_idx[p]
            ].src2_ready && (v.woken[v.oldest_idx[p]].op.store ||
                             (v.woken[v.oldest_idx[p]].op.load && !v.older_store_block));
      end
    end
    v.sel_found[0] = 1'b0;
    v.sel_found[1] = 1'b0;
    if (v.oldest_found[0] && v.oldest_ready[0]) begin
      if (!v.port_busy[0]) begin
        ...
        v.sel_found[0] = 1'b1;
      end else if (!v.port_busy[1]) begin
    ...
    for (int i = 0; i < RS_MEM_DEPTH; i++) begin
      v.slot_free[i] = (!v.woken[i].valid ||
                        (v.sel_found[0] && (v.sel_idx[0] == MEM_ADDR_BITS'(unsigned'(i)))) ||
          ...
          for (int m = 0; m < MEM_BANK_ENTRIES; m++) begin
            if (rs_in.alloc[k] && rin.free_found[k] &&
              ...
              array[m*ISSUE_WIDTH+k] <= rs_in.entry[k];
                         ...
                         !(rin.sel_found[0] &&
                           (rin.sel_idx[0] == MEM_ADDR_BITS'(unsigned'(m * ISSUE_WIDTH + k)))) &&
                         !(rin.sel_found[1] && (
                           rin.sel_idx[1] == MEM_ADDR_BITS'(unsigned'(m * ISSUE_WIDTH + k))))) begin
